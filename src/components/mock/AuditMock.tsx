...
export const AuditMock = ({ className = "" }: { className?: string }) => {
  return (
    <div className={`bg-background font-mono ${className}`}>
      <div className="p-3 border-b border-border flex items-center justify-between font-sans">
        <div>
          <div className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Audit log</div>
          <div className="text-sm font-medium tracking-tight">Workspace · Acme Retail</div>
        </div>
      ...
      <ul className="divide-y divide-border">
        {events.map((e, i) => (
          <motion.li
            key={i}
            className="grid grid-cols-12 gap-2 px-3 py-2 text-[10.5px]"
          ...
          >
            ...
            <span className="col-span-3">{e.u}</span>
            <span className="col-span-3 text-foreground/80">{e.a}</span>
